    ...
    <div className="space-y-5">
      <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
        {stats.map(({ label, value, icon: Icon, color, trend }) => (
          <div key={label} className="rounded-[18px] border border-slate-200 bg-[#f7f6f2] p-4 shadow-sm">
      ...
      <div className="rounded-[20px] border border-slate-200 bg-[#f7f6f2] p-4 shadow-sm">
        <div className="mb-5 flex items-center justify-between gap-3">
          <div>
            <h2 className="text-[16px] font-semibold text-slate-800">Revenue Overview</h2>
            <p className="text-xs text-slate-500">Year 2025 Performance</p>
          </div>
          ...
          <div className="flex items-center gap-2 rounded-full border border-slate-200 bg-white px-3 py-1.5 text-[11px] font-medium text-slate-600">
            <button className="rounded-full bg-slate-100 px-2 py-1">12 Months</button>
            <button className="rounded-full px-2 py-1">6 Months</button>
            <button className="rounded-full px-2 py-1">30 Days</button>
          </div>
        </div>

        <div className="h-[280px]">
          <ResponsiveContainer width="100%" height="100%">
            <BarChart data={revenueOverview} barGap={8}>
            ...
            <tbody>
              {activityRows.map((row) => (
                <tr key={row.id} className="border-t border-slate-200">
                  ...
                  <td className="px-4 py-4">
                    <span className={`rounded-full px-2.5 py-1 text-[11px] font-medium ${row.status === "Completed" ? "bg-emerald-50 text-emerald-700" : "bg-amber-50 text-amber-700"}`}>
                      {row.status}
                    </span>
                  </td>
